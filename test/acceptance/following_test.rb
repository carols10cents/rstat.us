...
describe "following" do
  ...
  describe "on rstat.us using the remote form" do
    before do
      log_in_as_some_user
      visit "/"
      click_link "Follow Remote User"

      VCR.use_cassette('subscribe_remote_rstatus') do
        fill_in 'subscribe_to', :with => "carols10cents@rstat.us"
        click_button "Follow"
      end
    end


    it "follows users on other rstat.us sites" do
      within flash do
        assert has_content? "Now following carols10cents."
      end
      assert "/", current_path
    end
  end
